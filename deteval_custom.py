...
def parse_args():
    ...
    # Conventional args
    parser.add_argument("--gt_file", default="../data/medical/ufo/fold1/fold1_val.json")
    parser.add_argument("--pred_file", default="../code/predictions/val_output.csv")
...
def default_evaluation_params():
    ...
    """
    return {
        "AREA_RECALL_CONSTRAINT": 0.8,  # threshold
...
def calc_deteval_metrics(
    ...
    for sample_name in gt_bboxes_dict:
        ...
        perSampleMetrics[sample_name] = {
            ...
            "one to many": ctn_otm,
            "many to one": ctn_mto,
            "det_bboxes/gt_bboxes": (len(detPolPoints) - len(detDontCareRectsNum))
            / (len(gtPolPoints) - len(gtDontCareRectsNum)),
        }
    ...
    num_of_samples = len(perSampleMetrics)

    ##oto, otm, mto
    oto, otm, mto = 0, 0, 0
    box_ratio = 0
    for sample in perSampleMetrics:
        oto += perSampleMetrics[sample]["one to one"]
        otm += perSampleMetrics[sample]["one to many"]
        mto += perSampleMetrics[sample]["many to one"]
        box_ratio += perSampleMetrics[sample]["det_bboxes/gt_bboxes"]

    methodMetrics = {
        "precision": methodPrecision,
        "recall": methodRecall,
        "hmean": methodHmean,
        "det_bboxes/gt_bboxes": box_ratio / num_of_samples,
        "one to one": oto,
        "one to many": otm,
        "many to one": mto,
    }
...
def create_gt_dict(gt_file_path):
    with open(gt_file_path) as json_reader:
        data = json.load(json_reader)

    gt_bboxes_dict = defaultdict(list)
    transcriptions_dict = defaultdict(list)

    for sample_name, bboxes in data["images"].items():
        for box in bboxes["words"].keys():
            gt_bboxes_dict[sample_name].append(bboxes["words"][box]["points"])
            transcriptions_dict[sample_name].append(
                bboxes["words"][box]["transcription"]
            )

    return gt_bboxes_dict, transcriptions_dict


def create_pred_dict_from_file(pred_file_path):
    with open(pred_file_path) as json_reader:
        data = json.load(json_reader)

    pred_bboxes_dict = defaultdict(list)

    for sample_name, bboxes in data["images"].items():
        for box in bboxes["words"].keys():
            pred_bboxes_dict[sample_name].append(bboxes["words"][box]["points"])

    return pred_bboxes_dict


def create_pred_dict(result):
    pred_bboxes_dict = defaultdict(list)

    for sample_name, bboxes in result["images"].items():
        for box in bboxes["words"].keys():
            pred_bboxes_dict[sample_name].append(bboxes["words"][box]["points"])
...
def main(args):
    gt_bboxes_dict, transcriptions_dict = create_gt_dict(args.gt_file)
    pred_bboxes_dict = create_pred_dict(args.pred_file)

    return calc_deteval_metrics(
        pred_bboxes_dict,
        gt_bboxes_dict,
        transcriptions_dict,
        eval_hparams=None,
